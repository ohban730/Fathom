...
- The Fathom view is a `WebviewViewProvider` registered under `viewsContainers.panel` (the bottom Panel area, alongside Terminal/Output/Debug Console) — see `vscode-extension/package.json`. It must stay there, not move to the sidebar/Activity Bar or become an editor-column `createWebviewPanel`/`ViewColumn` panel.
...
- `CHANGELOG.md` deliberately exists only in `vscode-extension/` (the Marketplace reads it from the package; the root README links to it). Do not add a second copy at the root — unlike the license files it changes every release, so a duplicate would drift.
