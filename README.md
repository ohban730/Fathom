...
- Ollamaを後からインストール/起動した場合も、ドロップダウンにモデルが並ぶので選べば実LLMに切り替わります。ドロップダウンが出てこない場合は、まだpull済みモデルが1つも無いか、Ollamaが起動していません(`ollama list`で確認)。それでも出ない場合はウィンドウを再読み込みしてください
